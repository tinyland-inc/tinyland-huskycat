...
def yaml_content(draw):
    ...
    values = draw(st.lists(st.one_of(st.integers(), st.text(), st.booleans()), min_size=1, max_size=10))
...
def dockerfile_content(draw):
    ...
    for _ in range(num_instructions - 2):
        instructions.append(draw(st.sampled_from([
            "RUN echo 'hello'",
            "COPY src/ /app/src/",
            "ENV KEY=value",
            "USER nobody",
...
class TestValidationEngineProperties:
    ...
    def test_python_validation_never_crashes(self, code):
        ...
        try:
            ...
            for result in results:
                ...
                assert result.tool in ["python-black", "python-flake8", "python-mypy", "python-bandit", "python-ruff"]
    ...
    def test_dockerfile_validation_properties(self, content):
        ...
        with tempfile.NamedTemporaryFile(mode='w', prefix='Dockerfile', delete=False) as f:
            f.write(content)
            f.flush()
            filepath = Path(f.name)
...
class TestValidationResultProperties:
    ...
    def test_validation_result_serialization(self, tool, filepath, success, messages, fixed, duration_ms):
...
class TestEngineInvariants:
    ...
    def test_unknown_tool_handling(self, tool_name):
        ...
        assume(tool_name not in ["python-black", "python-flake8", "js-eslint"])  # Skip known tools
