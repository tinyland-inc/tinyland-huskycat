...
'''
    ...
    def test_property_based_test_execution(self):
        ...
        for pbt_file in pbt_files:
            ...
            if test_path.exists():
                result = subprocess.run([
                    sys.executable, "-m", "pytest", 
                    str(test_path), "--tb=short", "-x"  # Stop on first failure
                ], cwd=PROJECT_ROOT, capture_output=True, text=True, timeout=60)
                
                # Should not crash (returncode 0 or 5 for no tests)
                assert result.returncode in [0, 5], f"PBT test {pbt_file} crashed: {result.stderr}"
    ...
    def test_makefile_test_target_execution(self):
        ...
        result = subprocess.run(["make", "clean-cache"], 
                              cwd=PROJECT_ROOT, capture_output=True, text=True, timeout=60)
