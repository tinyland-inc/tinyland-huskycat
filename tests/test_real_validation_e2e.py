...
""")
        ...
        found_issues = any(not result.success for result in results if hasattr(result, 'success'))
        if not found_issues:
            # Check legacy result format
            found_issues = any(result.messages for result in results if hasattr(result, 'messages'))
    ...
    def test_javascript_validation_flow(self, temp_project: Path):
        ...
        js_file.write_text("""const x=1+2;const y=3+4;
...
class TestGitHookIntegration:
    ...
    def git_project(self, isolated_dir: Path) -> Path:
        ...
        pre_commit_hook.write_text("""#!/usr/bin/env sh
...
""")
    ...
    def test_commit_msg_validation(self, git_project: Path):
        ...
        commit_msg_hook.write_text("""#!/usr/bin/env sh
. "$(dirname -- "$0")/_/husky.sh"
    ...
    def test_mcp_server_tools_list(self):
        ...
        try:
            ...
            for tool in expected_tools:
                ...
                pytest.fail(f"None of expected tools {expected_tools} found in {tool_names}")
...
class TestValidationScriptIntegration:
    ...
    def test_comprehensive_lint_script(self, scripts_dir: Optional[Path], temp_project: Path):
