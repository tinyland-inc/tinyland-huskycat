...
class ContainerTestHelper:
    ...
    def build_image(self, dockerfile_path: Path, tag: str, context_dir: Path) -> bool:
        ...
        cmd = [self.runtime, "build", "-f", str(dockerfile_path), "-t", tag, str(context_dir)]
    ...
    def run_container(self, image: str, command: Optional[List[str]] = None, 
...
class TestContainerBuild:
    ...
    def test_container_has_required_tools(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        for tool_cmd in tools_to_check:
            ...
            if result.returncode != 0:
                assert "command not found" not in result.stderr.lower(), f"Tool missing: {tool_cmd}"
    
    def test_container_python_environment(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        for import_test in python_imports:
            result = container_helper.run_container(built_image, ["python3", "-c", import_test])
...
class TestContainerSecurity:
    ...
    def test_runs_as_non_root(self, container_helper: ContainerTestHelper, built_image: str):
    ...
    def test_user_permissions(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        result = container_helper.run_container(
            built_image,
            ["sh", "-c", "touch /etc/test_file"]
        )
        assert result.returncode != 0, "User should not be able to write to /etc"
    
    def test_no_package_managers(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        for pm in package_managers:
            ...
            if result.returncode == 0:
                ...
                install_result = container_helper.run_container(
                    built_image, 
                    [pm, "update" if pm == "apk" else "list"]
                )
                # This is acceptable - tool exists but user can't use it
    
    def test_filesystem_permissions(self, container_helper: ContainerTestHelper, built_image: str):
...
class TestContainerFunctionality:
    ...
    def test_help_command(self, container_helper: ContainerTestHelper, built_image: str):
    ...
    def test_python_file_validation(self, container_helper: ContainerTestHelper, built_image: str):
...
""")
        ...
        try:
            ...
            result = container_helper.run_container(
                built_image,
                ["python3", "-c", f"import py_compile; py_compile.compile('/workspace/{temp_file.name}')"],
                volumes=volumes
            )
    ...
    def test_validation_tools_integration(self, container_helper: ContainerTestHelper, built_image: str):
...
""")
            ...
            result = container_helper.run_container(
                built_image,
                ["black", "--check", "/workspace/test.py"],
                volumes=volumes
            )
            # Black might not be configured to work this way, so just check it doesn't crash
            assert "Traceback" not in result.stderr
    
    def test_workspace_isolation(self, container_helper: ContainerTestHelper, built_image: str):
...
class TestContainerPerformance:
    ...
    @pytest.mark.slow
    def test_container_startup_time(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        assert startup_time < 10.0, f"Container startup took too long: {startup_time:.2f}s"
        assert result.returncode == 0
        assert "ready" in result.stdout
    
    @pytest.mark.slow
    def test_validation_performance_in_container(self, container_helper: ContainerTestHelper, built_image: str):
...
""")
            ...
            result = container_helper.run_container(
                built_image,
                ["sh", "-c", "for f in /workspace/*.py; do python3 -m py_compile \"$f\" || exit 1; done"],
                volumes=volumes
            )
    ...
    def test_container_resource_usage(self, container_helper: ContainerTestHelper, built_image: str):
...
class TestContainerHealthCheck:
    ...
    def test_health_check_endpoint(self, container_helper: ContainerTestHelper, built_image: str):
    ...
    def test_container_readiness(self, container_helper: ContainerTestHelper, built_image: str):
...
class TestContainerIntegration:
    ...
    def test_git_integration_in_container(self, container_helper: ContainerTestHelper, built_image: str):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.returncode == 0 or "not a git repository" not in result.stderr.lower()
    
    @pytest.mark.integration
    def test_file_permissions_consistency(self, container_helper: ContainerTestHelper, built_image: str):
