...
class TestMCPProtocolCompliance:
    ...
    def test_invalid_method_handling(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
    ...
    def test_error_response_structure(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
        """Test error response structure compliance."""
        # Trigger an error with invalid tool call
        request = mcp_client.create_request("tools/call", {
...
@pytest.mark.skipif(not (HAS_MCP_SERVER and HAS_VALIDATION), reason="MCP or validation not available")  
class TestMCPValidationIntegration:
    """Test MCP server integration with validation tools."""
    
    def test_validate_tool_python(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
...
""")
    ...
    def test_validate_black_tool(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
        """Test Black validation through MCP."""
        python_file = temp_workspace / "format_test.py"
        python_file.write_text("""def unformatted(x,y):
    return x+y""")
    ...
    def test_validate_multiple_files(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
...
""")
    ...
    def test_fix_mode_integration(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
    ...
    def test_validation_with_syntax_errors(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
...
""")
        ...
        if "result" in response:
            ...
            assert "error" in result_str or "syntax" in result_str or "invalid" in result_str
    
    def test_nonexistent_file_handling(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
        ...
        if "error" in response:
            error = response["error"]
            assert "not found" in error["message"].lower() or "no such file" in error["message"].lower()
...
class TestMCPServerRobustness:
    ...
    @pytest.mark.skipif(not HAS_MCP_SERVER, reason="MCP server not available")
    def test_concurrent_requests(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
    ...
    @pytest.mark.skipif(not HAS_MCP_SERVER, reason="MCP server not available")
    def test_resource_limits(self, mcp_server: MCPServer, mcp_client: MCPTestClient, temp_workspace: Path):
...
class TestMCPToolDiscovery:
    ...
    def test_tool_metadata_completeness(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
    ...
    def test_validation_tools_available(self, mcp_server: MCPServer, mcp_client: MCPTestClient):
        ...
        if "result" in response and "tools" in response["result"]:
            ...
            assert len(available_tools) > 0, f"No expected validation tools found. Available: {tool_names}"
    ...
    def test_stdio_protocol_compliance(self):
        ...
        for script in mcp_scripts:
            if script.exists():
                found_script = True
                # Check if executable
                assert script.stat().st_mode & 0o111, f"MCP server script {script} is not executable"
