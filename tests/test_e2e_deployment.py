    ...
    def test_container_startup_and_health(self, docker_client):
        ...
        try:
            ...
            container = docker_client.containers.run(
                ...
                environment={"NODE_ENV": "test", "MCP_SERVER_TOKEN": "test-token"},
            )
            ...
            if result.returncode == 0:
                try:
                    ...
                    assert (
                        ...
                        or "running" in status_result.stdout.lower()
                    )
...
class TestConfigurationDeployment:
    ...
    def test_script_deployment(self, fresh_environment: Path):
        ...
        for script_file in test_scripts_dir.glob("*.sh"):
            ...
            assert os.access(
                script_file, os.X_OK
            ), f"Script {script_file.name} not executable"
            ...
            assert (
                result.returncode == 0
            ), f"Syntax error in {script_file.name}: {result.stderr}"
...
class TestRollbackAndRecovery:
    ...
    def test_service_recovery(self):
        ...
        if not recovery_script.exists():
            ...
            result = subprocess.run(
                ["bash", "-n", str(recovery_script)], capture_output=True, text=True
            )
            assert (
                result.returncode == 0
            ), f"Recovery script has syntax errors: {result.stderr}"
