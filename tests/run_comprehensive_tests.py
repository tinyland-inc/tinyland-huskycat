...
class TestRunner:
    ...
    def run_command(self, command: List[str], cwd: Optional[Path] = None, 
                   timeout: Optional[int] = None, env: Optional[Dict] = None) -> Tuple[int, str, str, float]:
    ...
    def execute_test(self, phase: TestPhase, name: str, command: List[str], 
                    cwd: Optional[Path] = None, timeout: Optional[int] = None,
                    env: Optional[Dict] = None) -> TestResult:
    ...
    def run_integration_tests(self) -> List[TestResult]:
        ...
        if git_hooks_test.exists():
            result = self.execute_test(
                ...
                "Git Hooks Integration",
                ["python3", "-m", "pytest", str(git_hooks_test), "-v", "-m", "integration"],
                timeout=300
            )
            results.append(result)
        ...
        if mcp_integration_test.exists():
            result = self.execute_test(
                ...
                ["python3", "-m", "pytest", str(mcp_integration_test), "-v", "-m", "integration"],
                timeout=300
            )
            results.append(result)
        ...
        if real_validation_test.exists():
            result = self.execute_test(
                TestPhase.INTEGRATION,
                "Real Validation Flow",
                ["python3", "-m", "pytest", str(real_validation_test), "-v", "-m", "integration"],
                timeout=300
            )
            results.append(result)
    ...
    def run_container_tests(self) -> List[TestResult]:
        ...
        if container_test.exists():
            result = self.execute_test(
                ...
                ["python3", "-m", "pytest", str(container_test), "-v", "-m", "container"],
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        for phase, phase_results in by_phase.items():
            phase_passed = len([r for r in phase_results if r.success and not r.skipped])
            phase_failed = len([r for r in phase_results if not r.success and not r.skipped])
    ...
    def print_report(self):
        ...
        for phase, stats in report["by_phase"].items():
            print(f"   {phase.upper()}:")
            print(f"      Tests: {stats['total']} | Passed: {stats['passed']} | Failed: {stats['failed']} | Skipped: {stats['skipped']}")
    ...
    def run_all_tests(self, phases: Optional[List[TestPhase]] = None):
        """Run all test phases."""
        if phases is None:
            phases = [TestPhase.UNIT, TestPhase.INTEGRATION, TestPhase.E2E, TestPhase.CONTAINER]
