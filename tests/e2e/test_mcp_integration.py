...
class TestMCPServerIntegration:
    ...
    def test_mcp_server_tools_endpoint(
        self, mcp_server_process: Optional[subprocess.Popen]
    ):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                ), "Tools endpoint should return list or dict"
                ...
                found_tools = [
                    ...
                    if any(tool in name for name in tool_names)
                ]
                assert (
                    len(found_tools) > 0
                ), f"No expected tools found. Available: {tool_names}"
    ...
    def test_python_code_validation_integration(
        self,
        ...
        try:
            ...
            if good_code_response.status_code == 200:
                ...
                assert (
                    "validation_results" in result or "results" in result
...
"""
        ...
        try:
            response = requests.post(
                ...
                json={"code": insecure_code, "tools": ["security_bandit_scan"]},
                timeout=30,
            )
            ...
            if response.status_code == 200:
                ...
                if "security_issues" in result or "validation_results" in result:
                    ...
                    assert (
                        has_security_issues
                    ), "Expected security issues to be detected"

        except requests.RequestException:
            pytest.skip("MCP security validation endpoint not available")

    def test_yaml_validation_integration(
        self, mcp_server_process: Optional[subprocess.Popen]
    ):
...
"""
        ...
        try:
            ...
            valid_response = requests.post(
                ...
                json={"content": valid_yaml, "tools": ["yaml_lint"]},
                timeout=30,
            )
            ...
            invalid_response = requests.post(
                "http://localhost:8080/validate/yaml",
                json={"content": invalid_yaml, "tools": ["yaml_lint"]},
                timeout=30,
            )
...
class TestMCPServerDeployment:
    ...
    def test_mcp_server_container_deployment(self):
        ...
        try:
            ...
            try:
                ...
                ), f"Container not running: {container.status}"
                ...
                if port_info:
                    ...
                    try:
                        response = requests.get(
                            f"http://localhost:{host_port}/health", timeout=10
                        )
                        if response.status_code == 200:
                            health_data = response.json()
    ...
    def test_mcp_server_configuration_management(self):
        ...
        config_files = ["mcp-config.json", "config/mcp.yml", ".mcp-config.yaml"]
        ...
        if config_found:
            ...
            if isinstance(config, dict):
                ...
                assert (
                    len(found_keys) > 0
                ), f"No essential config keys found: {list(config.keys())}"
        else:
...
class TestMCPPerformance:
    ...
    def test_validation_performance(self):
        ...
        for size, code in test_cases:
            ...
            assert (
                duration < max_times[size]
            ), f"{size} code validation took too long: {duration:.2f}s"
    ...
    def test_concurrent_request_handling(self):
        ...
        assert (
            max_duration < 2.0
        ), f"Concurrent request took too long: {max_duration:.2f}s"
