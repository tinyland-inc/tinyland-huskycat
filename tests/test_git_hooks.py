...
'''
        ...
        if hook_path.exists():
            ...
            assert result.returncode in [
                0,
                1,
            ], f"Hook failed unexpectedly: {result.stderr}"
...
'''
            ...
            subprocess.run(
                ["git", "add", f"test_file_{i}.py"], cwd=test_repo, check=True
            )
        ...
        if husky_dir.exists():
            for hook_file in husky_dir.glob("*"):
                if hook_file.is_file() and not hook_file.name.startswith("_"):
                    ...
                    assert os.access(
                        hook_file, os.X_OK
                    ), f"Hook {hook_file} is not executable"
