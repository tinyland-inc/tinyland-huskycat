...
def create_parser():
    ...
    parser.add_argument(
        "--config-dir",
        type=Path,
        help="Configuration directory (default: ~/.huskycat)"
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate",
        help="Run validation on files"
    )
    validate_parser.add_argument(
        "files",
        nargs="*",
        help="Files to validate (default: staged files)"
    )
    validate_parser.add_argument(
        "--staged",
        action="store_true",
        help="Validate only staged git files"
    )
    validate_parser.add_argument(
        "--all",
        action="store_true",
    ...
    install_parser = subparsers.add_parser(
        "install",
        help="Install HuskyCat and dependencies"
    )
    ...
    schemas_parser = subparsers.add_parser(
        "update-schemas",
        help="Update validation schemas from official sources"
    )
    ...
    ci_parser = subparsers.add_parser(
        "ci-validate",
        help="Validate CI configuration files"
    )
    ci_parser.add_argument(
        "files",
        nargs="*",
        help="CI files to validate"
    )
    
    # Auto-DevOps command
    autodevops_parser = subparsers.add_parser(
        "auto-devops",
        help="Validate Auto-DevOps Helm charts and Kubernetes manifests"
    )
    ...
    status_parser = subparsers.add_parser(
        "status",
        help="Show HuskyCat status and configuration"
    )
